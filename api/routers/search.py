...
async def similar(
    ...
    """Return chunks most similar to the given chunk using its stored embedding."""
    emb_result = await db.execute(
        text("SELECT embedding FROM segment_chunks WHERE id = :id"),
        {"id": str(chunk_id)},
    )
    row = emb_result.fetchone()
    if row is None or row[0] is None:
        raise HTTPException(
            status_code=404, detail="Chunk not found or has no embedding"
        )

    vec_str = str(row[0])  # pgvector returns a string like '[0.1,0.2,...]'
    ...
    rows_result = await db.execute(
        text("""
            ...
            LIMIT :limit
        """),
        {"vec": vec_str, "chunk_id": str(chunk_id), "limit": limit},
    )
    raw = rows_result.mappings().all()
